...
const betterHoldDrag = (aFrameInstance) => {
  const component = {
    schema: {
      cameraId: {
        default: "camera",
    ...
    init() {
      ...
      const innerInit = function () {
        ...
        if (!this.camera) {
          throw new Error(
            `[better-hold-drag] Couldn't find camera with id '${this.data.cameraId}'`
          );
        }
        ...
        if (!this.ground) {
          throw new Error(
            `[better-hold-drag] Couldn't find ground with id '${this.data.groundId}'`
          );
        }
        this.internalState = {
          fingerDown: false,
    ...
    tick() {
      if (this.internalState?.dragging) {
        ...
        if (this.internalState.positionRaw) {
          ...
          const expRecords = this.internalState.raycaster.intersectObject(
        ...
        if (!d) {
          //eslint-disable-next-line no-undef
          d = this.camera.object3D.localToWorld(
  ...
  aFrameInstance.registerComponent("better-hold-drag", component);
};
...
export { betterHoldDrag, hologram4dsComponent, hologram4dsPrimitive };
